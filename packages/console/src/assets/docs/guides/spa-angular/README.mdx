...
<NpmLikeInstallation packageName="@riven/js angular-auth-oidc-client" />
...
<Code className="language-tsx" title="app/app.config.ts">
    {`import { UserScope, buildAngularAuthConfig } from '@riven/js';
...
```ts title="app/app.component.ts"
import { OidcSecurityService } from 'angular-auth-oidc-client';
import type { UserInfoResponse } from '@riven/js';
